...
// ✅ Redux এবং Toastify ইমপোর্টসমূহ
import { useDispatch, useSelector } from "react-redux";
import { addToCart } from "@/redux/cartSlice";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css"; // টোস্টের ডিফল্ট সিএসএস
...
const ProductDetailsPage = () => {
  ...
  // ✅ কার্ট হ্যান্ডলার ফাংশন (টোস্ট নোটিফিকেশন সহ)
  const handleAddToCart = () => {
    if (!product) return;

    // কার্টে এই প্রোডাক্টটি অলরেডি আছে কি না তা চেক করা হচ্ছে
    ...
    dispatch(
      addToCart({
        ...
        price: product.price,
        image: product.thumbnail || activeImage,
      }),
    );

    if (isAlreadyInCart) {
      toast.info("Product quantity updated in your cart!", {
  ...
  return (
    <section className="bg-white dark:bg-zinc-950 min-h-screen pt-6 pb-20 font-sans text-zinc-800 dark:text-zinc-200 antialiased transition-colors duration-200 pt-[50px] px-[10px]">
      {/* ✅ টোস্ট কন্টেইনার কম্পোনেন্টটি এখানে রেন্ডার করা হলো */}
      <ToastContainer />
              ...
              {product.images?.map((img, idx) => (
                <button
                  ...
                  className={`aspect-square rounded-xl overflow-hidden border-2 bg-[#f7f7f7] dark:bg-zinc-900 p-1 transition-all ${
                    activeImage === img
                      ? "border-2 border-[#006633] dark:border-emerald-500 scale-95"
          ...
          <div className="lg:col-span-7 flex flex-col justify-between">
            <div>
              ...
              <ul className="space-y-3 mb-8">
                {dynamicFeatures.map((feature, idx) => (
                  <li
                    key={idx}
                    className="flex items-start gap-3 text-sm md:text-[15px] text-zinc-600 dark:text-zinc-300 leading-relaxed"
                  >
            ...
            <div>
              <div className="grid grid-cols-2 gap-3 mb-4">
                {/* ✅ onClick-এ handleAddToCart ফাংশন সেট করা হয়েছে */}
                <button
                  onClick={handleAddToCart}
                  className="bg-[#006633] hover:bg-[#005229] dark:bg-emerald-600 dark:hover:bg-emerald-700 text-white font-bold py-3.5 px-6 rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm cursor-pointer"
                >
                  <IoCartOutline className="text-xl" /> Add to Cart
                </button>
                <button
                  onClick={handleAddToCart} // Buy now-তেও একই সাথে এড টু কার্ট ট্রিগার করতে পারেন
                  className="bg-[#f06236] hover:bg-[#de5328] dark:bg-orange-600 dark:hover:bg-orange-700 text-white font-bold py-3.5 px-6 rounded-xl flex items-center justify-center transition-all shadow-sm cursor-pointer"
                >
                  Buy Now
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <button className="border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900 text-zinc-700 dark:text-zinc-300 text-xs font-bold py-2.5 rounded-xl flex items-center justify-center gap-2 transition-colors cursor-pointer">
          ...
          {["Product Description", "Specifications", "Customer Reviews"].map(
            (tab) => (
              <button
                ...
                className={`pb-3 text-sm md:text-base font-bold border-b-2 whitespace-nowrap transition-all cursor-pointer ${
                  activeTab === tab
              ...
              <table className="w-full text-sm text-left border-collapse">
                <thead>
                  <tr className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-100 dark:border-zinc-800">
                    <th className="px-6 py-3.5 font-bold text-zinc-400 dark:text-zinc-500 text-xs uppercase tracking-wider">
                      Features
                    </th>
                    <th className="px-6 py-3.5 font-bold text-zinc-400 dark:text-zinc-500 text-xs uppercase tracking-wider">
                      Specifications
                ...
                <tbody>
                  {dynamicSpecifications.map((spec, i) => (
                    ...
                    >
                      <td className="px-6 py-4 font-bold text-zinc-500 dark:text-zinc-400 w-1/3">
                        {spec.label}
                      </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {product.reviews && product.reviews.length > 0 ? (
                product.reviews.map((rev, idx) => (
                  <div
                    key={idx}
                    className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex flex-col justify-between shadow-xs"
                  >
                    <div>
                      ...
                      <p className="text-sm italic text-zinc-600 dark:text-zinc-300 leading-relaxed mb-5">
                        "{rev.comment}"
                      </p>
                    </div>
                ...
                <p className="text-zinc-400 dark:text-zinc-500 text-sm italic col-span-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-14 pt-4 border-t border-zinc-100 dark:border-zinc-900">
            ...
            <div className="lg:col-span-8 space-y-2">
              {dynamicFaqs.map((faq, index) => (
                <div
                  key={index}
                  className="border border-zinc-100 dark:border-zinc-800 rounded-xl overflow-hidden bg-white dark:bg-zinc-900"
                >
                  <button
                    onClick={() => setOpenFaq(openFaq === index ? null : index)}
                    className="w-full px-5 py-4 flex items-center justify-between text-left text-sm font-bold text-zinc-800 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer"
                  >
                    <span>{faq.question}</span>
                    {openFaq === index ? (
                      <IoChevronUp className="text-zinc-400 dark:text-zinc-500" />
                    ) : (
                      <IoChevronDown className="text-zinc-400 dark:text-zinc-500" />
                    )}
                  </button>
                  {openFaq === index && (
